...
export default function AnulacionesItemsPage() {
  const { selectedSucursalId } = useSucursal();
  const [items, setItems] = useState<AnulacionItemRow[]>([]);
  ...
  const loadAnulaciones = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get('/reportes/anulaciones/items', {
        params: {
      ...
      console.error('Error al cargar el reporte de anulaciones por ítem:', err);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h3 className="text-base font-bold text-pearl-900">Anulaciones por Ítem</h3>
          <p className="text-xs text-pearl-400">
            Cada artículo ya comandado que fue anulado, con la orden a la que pertenece
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl border border-pearl-100 p-5 shadow-sm">
          <span className="text-[10px] font-bold text-pearl-400 uppercase tracking-widest block">Ítems anulados (página)</span>
      ...
      {loading && items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-pearl-400">
          <Ban size={40} className="animate-pulse mb-3" />
            ...
            <table className="w-full text-left border-collapse text-pearl-700">
              <thead>
                <tr className="bg-ice-50 border-b border-pearl-100 text-pearl-500 font-semibold text-xs uppercase tracking-wider">
                  ...
                  <th className="py-4.5 px-6 text-right">Cantidad</th>
                ...
                {items.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="py-8 text-center text-pearl-400">
                      No se encontraron ítems anulados en el rango seleccionado.
                    </td>
                  </tr>
                ) : (
                  items.map(a => (
                    <tr key={a.id} className="hover:bg-ice-50/50 transition-colors">
                      ...
                      <td className="py-4 px-6 font-medium text-pearl-900">{a.producto}</td>
                      <td className="py-4 px-6 text-right tabular-nums">{a.cantidad}</td>
